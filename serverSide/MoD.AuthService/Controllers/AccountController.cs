...

{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public AccountController(UserManager<MoDUser> userManager,
            SignInManager<MoDUser> signInManager,RoleManager<IdentityRole> roleManager,
        ...
        public async Task<IActionResult> Login([FromBody] LoginDto model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                if (appUser.IsEnabled != true) //block is enabled
                {
                    var response = await GenerateJwtToken(model.Email, appUser);
                    return Ok(response);
                }
                return Unauthorized("Not authorized to login, contact admin");
